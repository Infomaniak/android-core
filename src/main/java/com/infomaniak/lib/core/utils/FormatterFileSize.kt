...
object FormatterFileSize {
    ...
    fun formatShortFileSize(context: Context, sizeBytes: Long, justValue: Boolean = false): String {
        return try {
            val bytesResult = formatBytes(context.resources, sizeBytes, FLAG_IEC_UNITS or FLAG_SHORTER)
            if (justValue) {
                bytesResult.value
            } else {
                context.getString(
    ...
    fun formatBytes(res: Resources, sizeBytes: Long, flags: Int): BytesResult {
        ...
        // Note:
        // We calculate the rounded long by ourselves, but still let String.format() compute the rounded value.
        // String.format("%f", 0.1) might not return "0.1" due to floating point errors.
        val roundFactor: Int
        ...
        // Note:
        // This might overflow if abs(result) >= Long.MAX_VALUE / 100, but that's like 80PB so it's okay (for now)...
        val roundedBytes = if (flags and FLAG_CALCULATE_ROUNDED == 0) {
            0
        } else {
            (result * roundFactor).roundToLong() * mult / roundFactor
        }
